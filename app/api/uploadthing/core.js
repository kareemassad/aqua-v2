...
export const ourFileRouter = {
  excelUploader: f(["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"])
    ...
    .onUploadComplete(async ({ file, metadata }) => {
      ...
      const headerIndex = rawData.findIndex(row => row.some(cell => cell !== ''));
      const headers = rawData[headerIndex];

      // Process the data
      const products = rawData.slice(headerIndex + 1)
        .filter(row => row.some(cell => cell !== '')) // Remove empty rows
        .map(row => {
          const product = {};
          headers.forEach((header, index) => {
            if (header) {
              product[header.trim()] = row[index] ? row[index].toString().trim() : '';
            }
          });
          return product;
        });
