...
export default function CollectionsPage() {
  ...
  const handleAddProducts = async (collectionId) => {
    try {
      ...
      if (added > 0 && duplicates > 0) {
        toast.info(`Added ${added} product(s). ${duplicates} product(s) were already in the collection.`);
      } else if (added > 0) {
        toast.success(`Added ${added} product(s) to the collection.`);
      } else if (duplicates > 0) {
        toast.warning(`All ${duplicates} product(s) were already in the collection.`);
      }
  ...
  const filteredCollections = collections.map(collection => ({
    ...collection,
    products: collection.products ? collection.products.filter(product => 
                ...
                <TableBody>
                  {collection.products && collection.products.map(product => (
                    <TableRow key={product._id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => handleRemoveProduct(collection._id, product._id)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </TableCell>
                    </TableRow>
